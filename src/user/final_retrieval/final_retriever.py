...
def retrieve_chunks(
    ...
    This function queries the Chroma collection for each chunk ID and constructs
    a string containing the source metadata and content for each retrieved chunk.
    The output is formatted for use as context in LLM prompts.
    ...
    Returns:
        str: Formatted context string with source and content for each chunk.
            ...
            Returns empty string if:
                ...
                - No chunks are found in the collection
    
    Raises:
        None: All exceptions are caught and logged. Failed chunks are skipped.
    
    Example:
        >>> chunks = ['doc_1_chunk_1', 'doc_2_chunk_1']
        >>> context = retrieve_chunks(vector_store, chunks)
        >>> print(context)
    ...
    Note:
        - Chunks are retrieved in the order provided (preserves ranking)
        - Missing chunks are skipped with a warning log
        - The function is fault-tolerant: if one chunk fails, others still process
    """
    ...
    for chunk_id in chunks:
        try:
            ...
            results = vector_store._collection.get(where={"chunk_id": chunk_id})
